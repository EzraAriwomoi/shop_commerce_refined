...
const NOTIFICATIONS_API_URL =
  "https://back-server-1.onrender.com/notifications/";
...
const Navbar = () => {
  ...
  const handleSignOut = async () => {
    ...
    try {
      const response = await fetch(
        "https://back-server-1.onrender.com/auth/logout",
        {
          method: "POST",
          headers: {
  ...
  const checkAuthStatus = async () => {
    ...
    try {
        ...
        if (response.ok) {
            const data = await response.json();
            console.log("Authentication status:", data);  // Should return { authenticated: true }
        } else {
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (event) => {
      ...
      if (
        ...
        !fabarsRef.current.contains(event.target)
      ) {
  ...
  return (
    <nav className="navbar">
      ...
      <div
        className={`navbar-content ${mobileMenuOpen ? "active" : ""}`}
        ref={mobileMenuRef}
      >
        {mobileMenuOpen ? (
            ...
            {isAuthenticated ? (
              <>
                ...
                <Link
                  ...
                  to="/myaccount"
                  className="link-to profile-link"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 512 512"
                ...
                <a
                  href="#"
                  className="logout-signin-mobile"
        ...
        <div className="navbar-icons">
          ...
          <div className="dropdown" ref={notificationsRef}>
            <button onClick={handleNotificationsClick} className="icon-button">
              <FaBell title="notifications" />
            </button>
            {notificationsOpen && (
              <NotificationMenu
