...
const CustomerSignInComponent = ({ onClose, onChangeView }) => {
  ...
  const handleSignIn = async (e) => {
    ...
    console.log("Sending sign-in data:", signInData);

    try {
        const response = await fetch("http://127.0.0.1:8000/api/users/sign_in/", {
            method: "POST",
            headers: {
  ...
  return (
    ...
    >
      <div
        className={`customer-sign-in-component ${isVisible ? "drop-down" : ""}`}
      >
        {loading && (
        ...
        <form onSubmit={handleSignIn} className="csc-form flex-col">
          ...
          <span
            onClick={() => onChangeView("reset-password")}
